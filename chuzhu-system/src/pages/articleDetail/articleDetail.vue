<template>
	<view class="articleDetail">
		<view class="content">
			...
			<view class="info">
				<view class="left">
					<image :src="articleDetail.user.avatar" mode=""></image>
...
<!--      文章主体-->
			<block v-for="(item, index) in articleDetail.extras" :key="index" class="cs-rich-text-X">
				<rich-text
                selectable='true'
                :nodes="item.content"
	...
	methods: {
        ...
        success: res=> {
          ...
          if(res.data.data.status_code === 200) {
            for (let i = 0; i < res.data.data.extras.length; i++) {
              res.data.data.extras[i].content = helper.formatRichText(res.data.data.extras[i].content);
            }
